...
// Product list moved to a shared module so multiple components can stay in sync
import { products as demoProducts } from "@/lib/demo-products";
...
export default function ProductDetailClient({ id }: { id: string }) {
  ...
  const product = demoProducts.find((p) => p.id === id) || demoProducts[1]; // Fallback to first product if not found
  const badges = product.badges ?? [];
  const productSummary = product.productSummary ?? {
    styleNo: "—",
    width: "—",
    height: "—",
    metalWeight: "—",
  ...
  const youMayAlsoLike = product.youMayAlsoLike ?? [];
  const includedWithPurchase = product.includedWithPurchase ?? [];
  const priceBreakup = product.priceBreakup ?? [
    { label: "Grand Total", value: `₹${product.currentPrice}`, isGrandTotal: true },
  ];
  const candereProduct = {
    id: product.id,
    name: product.name,
    ...
    youMayAlsoLike: youMayAlsoLike.map((r) => ({
      id: r.id,
      name: r.name,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-gray-900">
                ...
                {metalOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: getMetalGradient(opt.tone as string),
                      }}
                    >
      ...
      {/* Product Information (Candere UI) */}
      <CandereProductDetails product={candereProduct} />
      ...
      <section className="bg-[#e0f2f2] py-12 px-4 md:px-8 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 max-w-7xl mx-auto">
          {includedWithPurchase.map((item, index) => (
            <div
              key={index}
