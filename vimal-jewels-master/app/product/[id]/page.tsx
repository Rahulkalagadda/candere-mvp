...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const p = getProductById(id) ?? (getProductById("elenai-gold-hoop-earrings") as NonNullable<ReturnType<typeof getProductById>>)
  const safeProduct = {
    id: p.id,
    name: p.name,
    ...
    productSummary: p.productSummary
      ? {
          styleNo: p.productSummary.styleNo ?? undefined,
          width: p.productSummary.width ?? undefined,
          height: p.productSummary.height ?? undefined,
          metalWeight: p.productSummary.metalWeight ?? undefined,
          grossWeight: p.productSummary.grossWeight ?? undefined,
        }
      : undefined,
    metalDetails: p.metalDetails ?? undefined,
    priceBreakup:
      p.priceBreakup?.map((row) => ({
        label: row.label,
        value: row.value,
        originalValue: row.originalValue ?? undefined,
        isGrandTotal: Boolean(row.isGrandTotal),
      })) ?? [
        { label: "Grand Total", value: `₹${p.currentPrice}`, isGrandTotal: true },
      ],
    youMayAlsoLike: (p.youMayAlsoLike || []).map((r) => ({
      id: r.id,
      name: r.name,
